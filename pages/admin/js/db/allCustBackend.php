...
if(isset($_POST['id'])){
    ...
    if($sts == '0'){
        ...
        if($res2){
            ...
            $dbEmpId = $arr3['empId'];
            $query4 = "UPDATE `new_connection` SET `empId`='[value-1]' WHERE `custId` = $id";
...
if(isset($_POST['serchText'])){
    ...
    if($count >= 1){
        ...
        while($arr = mysqli_fetch_array($res)){
            ...
            $dbEmail = $arr['email'];
            $dbConSTS = $arr['cSTS'];
            if($dbConSTS == 'active' || $dbConSTS == 'offline'){
            $data .= '<tr id="tableData" data-widget="expandable-table" aria-expanded="false">
                            <td>'.$slno++.'</td>
                            <td>'.$dbName.'</td>
                            <td>'.$dbPhNo.'</td>
