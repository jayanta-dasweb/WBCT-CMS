...
if(isset($_POST['dummyVar'])){
    require './config.php';
    $data = '';
    $query = "SELECT * FROM `payment` JOIN `customer` ON payment.custId=customer.custId JOIN `new_connection` ON new_connection.custId=customer.custId JOIN `cable_operator` ON cable_operator.empId=new_connection.empId WHERE payment.payStatus = 'pending'";
...
if(isset($_POST['serchText'])){
    ...
    require './config.php';
    $data = '';
    $query = "SELECT * FROM `payment` JOIN `customer` ON payment.custId=customer.custId JOIN `new_connection` ON new_connection.custId=customer.custId JOIN `cable_operator` ON cable_operator.empId=new_connection.empId WHERE payment.payStatus = 'pending' AND payment.payNo LIKE '%".$searchText."%' OR customer.custName LIKE '%".$searchText."%' OR customer.STBNumber LIKE '%".$searchText."%' OR cable_operator.empName LIKE '%".$searchText."%'";
    $res = mysqli_query($conn,$query);
    $count = mysqli_num_rows($res);
    if($count >= 1){
        $data .= '<table class="table table-bordered table-head-fixed  table-hover">
