...
if(isset($_POST['dummyVar'])){
    ...
    $slno = 1;
    require './config.php';
    $query = "SELECT customer.phNumber As phNo , customer.* , complaints.* , cable_operator.* FROM `customer` JOIN `complaints` ON customer.custId=complaints.custId JOIN `cable_operator` ON cable_operator.empId=complaints.empId  WHERE complaints.status = 'resolved' ORDER BY complaints.complaintsId DESC";
