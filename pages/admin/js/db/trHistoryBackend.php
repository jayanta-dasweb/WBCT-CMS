...
if(isset($_POST['dummyVar'])){
    require './config.php';
    $data = '';
    $query = "SELECT * FROM `payment` JOIN `customer` ON payment.custId=customer.custId JOIN `new_connection` ON new_connection.custId=customer.custId JOIN `cable_operator` ON cable_operator.empId=new_connection.empId WHERE payment.payStatus = 'done'";
