...
if(isset($_POST['dummyVar'])){
    ...
    $query = "SELECT customer.phNumber As phNo , customer.* , complaints.* , cable_operator.* FROM `customer` JOIN `complaints` ON customer.custId=complaints.custId JOIN `cable_operator` ON cable_operator.empId=complaints.empId  WHERE customer.custId = $custId  ORDER BY complaints.complaintsId DESC";
...
if(isset($_POST['serchText'])){
    ...
    $slno = 1;
    require './config.php';
    $query = "SELECT customer.phNumber As phNo , customer.* , complaints.* , cable_operator.* FROM `customer` JOIN `complaints` ON customer.custId=complaints.custId JOIN `cable_operator` ON cable_operator.empId=complaints.empId  WHERE customer.custName LIKE '%".$searchText."%' OR customer.phNumber LIKE '%".$searchText."%' OR complaints.complaintNo  LIKE '%".$searchText."%' OR cable_operator.empName LIKE '%".$searchText."%' AND customer.custId = $custId ORDER BY complaints.complaintsId DESC";
    $res = mysqli_query($conn,$query);
    $count = mysqli_num_rows($res);
    if($count >= 1){
        $data .= '<table class="table table-bordered table-head-fixed  table-hover">
