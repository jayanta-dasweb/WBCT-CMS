...
if(isset($_POST['dummyVar'])){
    ...
    $id = $_POST['dummyVar'];
    require './config.php';
    $query = "SELECT customer.phNumber As phNo , customer.* , complaints.* , cable_operator.* FROM `customer` JOIN `complaints` ON customer.custId=complaints.custId JOIN `cable_operator` ON cable_operator.empId=complaints.empId  WHERE complaints.status = 'pending' AND cable_operator.empId = $id ORDER BY complaints.complaintsId DESC";
