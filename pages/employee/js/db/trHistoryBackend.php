...
if(isset($_POST['dummyVar'])){
    ...
    $sid = $_POST['dummyVar'];
    $data = '';
    $query = "SELECT * FROM `payment` JOIN `customer` ON payment.custId=customer.custId JOIN `new_connection` ON new_connection.custId=customer.custId JOIN `cable_operator` ON cable_operator.empId=new_connection.empId WHERE payment.payStatus = 'done' AND cable_operator.empId = $sid";
...
if(isset($_POST['serchText'])){
    ...
    require './config.php';
    $data = '';
    $query = "SELECT payment.*, customer.*, new_connection.empId AS emplId, new_connection.*, cable_operator.*  FROM `payment` JOIN `customer` ON payment.custId=customer.custId JOIN `new_connection` ON new_connection.custId=customer.custId JOIN `cable_operator` ON cable_operator.empId=new_connection.empId WHERE payment.payNo LIKE '%".$searchText."%' OR customer.custName LIKE '%".$searchText."%' OR customer.STBNumber LIKE '%".$searchText."%' OR cable_operator.empName LIKE '%".$searchText."%' AND payment.payStatus = 'done' AND new_connection.empId = $sid";
    $res = mysqli_query($conn,$query);
    $count = mysqli_num_rows($res);
    if($count >= 1){
        $data .= '<table class="table table-bordered table-head-fixed  table-hover">
