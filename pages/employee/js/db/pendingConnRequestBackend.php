...
if(isset($_POST['dummyVar'])){
    ...
    $data = '';
    $slno = 1;
    $query = "SELECT * FROM `new_connection` INNER JOIN `customer` ON new_connection.connId=customer.connId WHERE new_connection.connStatus='pending' AND new_connection.empId = $id  ORDER BY new_connection.connId DESC  ";
...
if(isset($_POST['serchText'])){
    ...
    $slno = 1;
    require "./config.php";
    $query = "SELECT * FROM `new_connection` INNER JOIN `customer` ON new_connection.connId=customer.connId WHERE `custName` LIKE '%".$searchText."%' OR `email` LIKE '%".$searchText."%' OR `phNumber` LIKE '%".$searchText."%' AND new_connection.connStatus='pending' AND new_connection.empId = ".$id." ORDER BY new_connection.connId DESC";
    $res = mysqli_query($conn,$query);
    $count = mysqli_num_rows($res);
    if($count >= 1){
        $data .= '<table class="table table-bordered table-head-fixed  table-hover">
